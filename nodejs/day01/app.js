...
const saramList = [
    {no:1001, name:"홍길동", dept:"기획부", grade:"대리"},
    {no:1002, name:"김길동", dept:"재정부", grade:"부장"},
    {no:1003, name:"박길동", dept:"인사부", grade:"사원"}
];
let noCnt=1004;
...
app.put("/saram", (req, res) => {
    ...
    const idx = saramList.findIndex((saram) => {
        return saram.no === req.body.no;
    })
    if (idx != -1) {
        saramList[idx].name=req.body.name;
        saramList[idx].dept=req.body.dept;
        saramList[idx].grade=req.body.grade;
    }
    res.send(saramList);
});
